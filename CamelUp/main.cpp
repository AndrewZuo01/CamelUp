...
extern "C"{
    ...
    void decision(Player* currentPlayer,std::vector<Horse*>& HorseList,std::vector<Horse*>& tempHorseList,Board* board){
        int choice;
        std::cout<<"Player "<< currentPlayer->numberIndex<<". Your decision for this round: "<<std::endl;
        std::cout<<"1 is choosing legwinner; 2 is guessing first horse; 3 is guessing last horse; 4 is placing trap;"<<std::endl;
        std::cin >> choice;
        //type check!
        switch(choice){
            case 1:
    ...
    void rewardPlayerAtLast(std::vector<Player*> PlayerList,std::vector<Horse*> HorseList,Board* board){
        ...
        for(auto p: PlayerList){
            std::cout<<"Player "<<p->numberIndex << " money is:"<<p->CurrentMoney<<std::endl;
        }
    }
    //start game
    int gameboardStart(){
